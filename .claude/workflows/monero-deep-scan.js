...
const researched = await parallel(cells.map((cell) => () => agent(
  [CONTEXT, '',
   ...
   'about where a check happens sends the NEXT review past the place a bug',
   'lives. Nothing is edited from this run: the report proposes and a human',
   'lands it.',
   'Not a place to note that a reference was USEFUL, and never a reason to',
   'read beyond your unit -- report what you met on the walk you were already',
   'making.',
   '',
...
if (!candidates.length) {
  return {
    ...
    next: [
      ...
      'the Monero tree, found by a reader who had just walked the code they',
      'describe. NOT a section and no heading: the harness strips the stamp before',
      'publishing and opens a pull request against this repo from it, so nothing',
      'about it reaches the maintainer reading the issue. Omit it when the array',
      'is empty.',
    ].join('\n'),
  }
}
...
return {
  ...
  next: [
    ...
    'macros.md is stale, and a section about our housekeeping in their issue is',
    'noise. Copy the entries verbatim into the `<!-- workflow-updates ... -->` JSON',
    'stamp immediately ABOVE the coverage stamp, keeping the four fields `file`,',
    '`says`, `correction` and `evidence`. The harness strips that stamp before',
    'publishing and hands it to a job that opens a pull request against THIS repo.',
    'That job never sees the pull request you reviewed, which is why the stamp',
    'carries typed fields rather than prose. Omit it when the array is empty.',
    '',
