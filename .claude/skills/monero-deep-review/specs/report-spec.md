...
**Severity comes back already decided.** The workflow lowered any
severity its agreeing verifiers rated below the proposal, and caps a confidence
...
**Order by severity, then by how many angles agreed.** People stop reading partway down.
...
- headings stay `### [SEVERITY] Title`, severity spelled
...
- `## Summary` sits above `## Findings` and holds prose only. A bracketed
  severity there labels the issue exactly as one in `## Coverage` would, for
  the same reason -- it is above the first `## Refuted`;
